...
class DeepConvNet:
    ...
    def __init__(self, input_dim=(1, 28, 28),
        ...
        pre_node_nums = cp.array([1*3*3, 16*3*3, 16*3*3, 32*3*3, 32*3*3, 64*3*3, 64*4*4, hidden_size])

        #weight_init_scales = np.sqrt(2.0 / pre_node_nums)  # ReLUを使う場合に推奨される初期値
        weight_init_scales = cp.sqrt(2.0 / pre_node_nums)  # ReLUを使う場合に推奨される初期値
        ...
        self.params['W7'] = weight_init_scales[6] * cp.random.randn(64*4*4, hidden_size)

        #self.params['b7'] = np.zeros(hidden_size)
        self.params['b7'] = cp.zeros(hidden_size)

        #self.params['W8'] = weight_init_scales[7] * np.random.randn(hidden_size, output_size)
        self.params['W8'] = weight_init_scales[7] * cp.random.randn(hidden_size, output_size)
